 ...
 *
 * Note:
 *  When Data Cache is enabled, ETH DMA Descriptors (DMARxDscrTab and DMATxDscrTab) and
 *  ETH frame buffers (Rx_Buff and TX_Buff) must be positioned in Non-cacheable memory.
 * -------------------------------------------------------------------------- */

/* History:
 *  Version 1.4
 *    Removed Data Cache maintenance (ETH DMA descriptors and frame buffers must
 *    be positioned in Non-cacheable memory).
 *  Version 1.3
...
static ETH_DMADescTypeDef DMARxDscrTab[ETH_RX_DESC_CNT]                     ; /* __MEMORY_AT(0x30040000); */
static ETH_DMADescTypeDef DMATxDscrTab[ETH_TX_DESC_CNT]                     ; /* __MEMORY_AT(0x30040060); */
static uint8_t            Rx_Buff     [ETH_RX_DESC_CNT][ETH_MAX_PACKET_SIZE]; /* __MEMORY_AT(0x30040200); */
static uint8_t            TX_Buff     [ETH_TX_DESC_CNT][ETH_MAX_PACKET_SIZE]; /* __MEMORY_AT(0x30042000); */
