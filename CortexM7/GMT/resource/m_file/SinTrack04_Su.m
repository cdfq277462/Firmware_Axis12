function traj = SinTrack04_Su(TMtool,Tstart,velocity,Ts,TimeScaling,LineSpace,StepRange,ScanRange)
...
for j=1:2
    ...
    for k=1:(2*K+1)
        ...
        Z= 0.0*ToolTranslz;
        Trans(:,k,j) = [ax;ay;az]+X+Y+Z; 
        ToolTrans(:,k,j)=TMtool(1:3,4)+Trans(:,k,j);
        Traj(:,k,j)=[Trans(:,k,j);Pstart(4:6)];  
        [Ls,gamma] = N_Stewart_IK(Traj(:,k,j),pp,bb);
...
%-------------------------------
traj = [traj0 traj1(:,:,1) traj1(:,:,2) traj2];
...
plot3(ToolTrans(1,1:end,1)',ToolTrans(2,1:end,1)',ToolTrans(3,1:end,1)','o'); 
plot3(ToolTrans(1,1:end,2)',ToolTrans(2,1:end,2)',ToolTrans(3,1:end,2)','o'); 
